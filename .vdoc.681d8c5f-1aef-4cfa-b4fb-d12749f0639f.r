...
#| code-summary: "Regressão logística com efeito aleatório para setor - Mulheres"
#| warning: false
#| message: false
...
#| code-summary: "Regressão logística com efeito aleatório para setor - Homens"
#| warning: false
#| message: false
...
#
#
#| label: Modelos AF global mulheres com desenho amostral
#| code-summary: "Regressão logística com desenho amostral complexo - Mulheres"
...
modelos <- function(exp_var) {
  ...
  assign(paste0("fit_glm_af_mulher_", exp_var), fit_glm_af, envir = .GlobalEnv)
  rotulo_atual <- setNames(list(as.character(rotulos[exp_var])), exp_var)
  fit_glm_af %>%
    tbl_regression(
      exponentiate = TRUE,
...
tabela_glm_af_mulheres <- map(exposicoes, modelos) %>%
  tbl_stack() %>%
  modify_spanning_header(
    c(estimate, conf.low, conf.high, p.value) ~ "**Global physical activity - Women**"
  )

tabela_glm_af_mulheres
#
#
#
#| label: Modelos AF global homens com desenho amostral
#| code-summary: "Regressão logística com desenho amostral complexo - Homens"
...
modelos <- function(exp_var) {
  ...
  assign(paste0("fit_glm_af_homem_", exp_var), fit_glm_af, envir = .GlobalEnv)
  rotulo_atual <- setNames(list(as.character(rotulos[exp_var])), exp_var)
  fit_glm_af %>%
    tbl_regression(
      exponentiate = TRUE,
...
tabela_glm_af_homens <- map(exposicoes, modelos) %>%
  tbl_stack() %>%
  modify_spanning_header(
    c(estimate, conf.low, conf.high, p.value) ~ "**Global physical activity - Men**"
  )

tabela_glm_af_homens
#
#
#
#
