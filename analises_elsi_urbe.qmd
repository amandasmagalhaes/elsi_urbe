...
```{r}
#| code-summary: "Padrão decimal e de milhar para português"
...
df %>%
  ...
  select(-p_val_raw) %>%
  knitr::kable(
    caption = "Teste de Normalidade (Shapiro-Wilk)",
...
df %>%
  ...
  select(-p_val_raw) %>%
  knitr::kable(
    caption = "Teste de Normalidade (Shapiro-Wilk) - Renda per capita",
    col.names = c("Auditoria virtual", "p-valor", "Resultado")
  )
```
